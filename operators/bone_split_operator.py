...
class OBJECT_OT_split_spine_shoulder(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # 收集切分前的权重总和用于验证
        pre_weights = self._collect_weights(context, obj, ["上半身2"])
        ...
        neck = edit_bones.get("首")
        if ub2:
            # 上半身2 占下半段，上半身3 占上半段（连接到首）
            mid = ub2.head.lerp(ub2.tail, 0.5)
            ub2_head = ub2.head.copy()
            ub2_tail = mid.copy()
            ub3_head = mid.copy()
            ub3_tail = ub2.tail.copy()
            ub2_parent = ub2.parent.name if ub2.parent else None
    ...
    def _redistribute_spine_weights(self, context, armature_obj):
        """将上半身2的权重按顶点到切分点的距离比例分配给上半身2和上半身3"""
        # 需要在 object mode 下操作
        ...
        for obj in context.scene.objects:
            ...
            for v in obj.data.vertices:
                ...
                # 设置新权重
                if ratio2 > 0:
                    vg2.add([v.index], w2 * ratio2, 'REPLACE')
                else:
                    vg2.remove([v.index])

                if ratio3 > 0:
                    vg3.add([v.index], w2 * ratio3, 'REPLACE')
